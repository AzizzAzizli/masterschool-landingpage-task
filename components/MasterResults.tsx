...
export const MasterResults = () => {
  const t = useTranslations('master-results');

  return (
    <section id="master-neticeler" className="section-padding  bg-background/20 dark:bg-background/80 relative overflow-hidden">
      {/* Decorative Elements */}
      <div className="absolute top-0 left-0 w-[400px] h-[400px] bg-accent/10 rounded-full blur-[100px] -translate-y-1/2 -translate-x-1/2 pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-[400px] h-[400px] bg-primary-500/10 rounded-full blur-[100px] translate-y-1/2 translate-x-1/2 pointer-events-none" />
              ...
              <SwiperSlide key={index} className="w-[280px]! sm:w-[300px]! md:w-[340px]!">
                <div className="glass-card flex flex-col items-center p-4 group transition-all duration-500 hover:shadow-2xl hover:border-primary-400 dark:hover:border-primary-500 hover:-translate-y-2">
                  <div className="relative w-full aspect-3/4 rounded-xl overflow-hidden shadow-inner">
                    <Image
                      src={img}
                      alt={`Magistratura Nəticəsi ${index + 1}`}
                      fill
